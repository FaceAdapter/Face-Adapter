...
class ResNetYoutu3DMM(Module):
    ...
    def __init__(self, input_size, output_dim, block, layers, zero_init_residual=True):
        """ Args:
            ...
            layers: layers in each block
        """
        super(ResNetYoutu3DMM, self).__init__()
...
class ResNet101_nofc(Module):
    ...
    def __init__(self, input_size, output_dim, use_last_fc=False, init_path=None):
        """ Args:
            ...
            layers: layers in each block
        """
        super(ResNet101_nofc, self).__init__()
        assert input_size[0] in [112, 224, 256], \
            "input_size should be [112, 112] or [224, 224]"
        func, last_dim = func_dict['resnet101']
        self.use_last_fc=use_last_fc
        backbone = func(use_last_fc=use_last_fc, num_classes=output_dim)
        if init_path and os.path.isfile(init_path):
            state_dict = filter_state_dict(torch.load(init_path, map_location='cpu'))
            backbone.load_state_dict(state_dict)
            print("Loading init recon %s from %s"%('resnet101', init_path))
        self.backbone = backbone
        if not use_last_fc:
            self.fianl_layers = nn.ModuleList([
                conv1x1(last_dim, 80, bias=True), # id
...
class ResNetYoutu101(Module):
    """ ResNet backbone
    """
    def __init__(self, input_size, output_dim, block, layers, zero_init_residual=True, use_last_fc=False):
        """ Args:
            input_size: input_size of backbone
            block: block function
            layers: layers in each block
        """
        super(ResNetYoutu101, self).__init__()
        assert input_size[0] in [112, 224, 256], \
            "input_size should be [112, 112] or [224, 224]"
        self.inplanes = 64
        # self.pre = Conv2d(215*2, 256*256, kernel_size=1)
...
def ResNet_50_RG(input_size, output_dim, **kwargs):
    """ Constructs a ResNet-50-RG model.
        """
    model = ResNetYoutu3DMM(input_size, output_dim, Bottleneck, [3, 4, 6, 3], **kwargs)
    return model

def ResNet_101_RG(input_size, output_dim, **kwargs):
    model = ResNetYoutu101(input_size, output_dim, Bottleneck, [3, 4, 23, 3], **kwargs)
    return model
...
def resnet101(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet('resnet101', Bottleneck_, [3, 4, 23, 3], pretrained, progress,
                   **kwargs)
